...
**Excepción al "todo vendorizado" de GitHub Pages:** `docs/index.html` carga Font Awesome (íconos) y `xlsx`/SheetJS (exportar a Excel) desde `cdnjs.cloudflare.com` en vez de vendorizarlos como Leaflet/Tesseract. Si el usuario abre la app sin internet antes de la primera carga exitosa, esos dos fallan (el resto — OCR, tabla, mapa offline, copiar — sigue funcionando). Pendiente de decisión: vendorizar ambos o dejarlo así.

### Tabla de resultados y mapa (`docs/js/app.js`)

- Cada celda de la tabla es un `<td contenteditable="true">` independiente (clic para editar). Esto tiene una trampa: la selección nativa del navegador **no puede extenderse de una celda a otra** (queda atrapada en la primera región editable donde empezó el arrastre) — por eso la selección de un rango tipo Excel (arrastrar de X a COTA en una o más filas) está implementada a mano: `mousedown`/`mousemove`/`mouseup` llevan un rango propio de fila/columna, lo resaltan con la clase CSS `.celda-sel`, y un listener de `copy` intercepta el portapapeles con el TSV del rango — pero solo si hay 2+ celdas seleccionadas; con una sola celda se deja pasar la copia nativa del navegador (para no romper copiar texto parcial dentro de una celda). Aparte existe el botón "📋 Copiar tabla", que siempre copia **todos** los registros como TSV sin importar la selección. En Streamlit, `st.data_editor` ya soporta selección de rango nativa (arrastrar + Ctrl+C, viene de glide-data-grid); el botón "Copiar tabla" ahí se agregó aparte, embebido vía `st.components.v1.html`, replicando la misma lógica TSV.
- Los checkboxes `.capa-toggle` (`data-tipo="POZO|VDC|DCP|TIS|DCA|SIN CLASIFICAR"`) filtran a la vez el mapa online (Leaflet + `leaflet.markercluster`) y el mapa offline (dibujado a mano en `<canvas>`, sin dependencias, siempre disponible sin internet). Si se agrega o renombra un tipo de ensayo hay que tocar `TIPOS`/`SIMBOLOGIA` en `parser.js` **y** estos checkboxes en `docs/index.html` a la vez, o el filtro deja puntos huérfanos sin capa (fue justo la causa de un bug de "puntos que no aparecen en el mapa": comparar por el string completo de `Ensayo` sin pasarlo por `extraerTipoNumero`/`extraer_tipo_numero` primero).
